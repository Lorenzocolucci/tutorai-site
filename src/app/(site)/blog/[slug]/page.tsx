...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  if (!post) {
    return {
      title: 'Articolo Non Trovato | TutorAI',
  ...
  return {
    ...
    alternates: {
      canonical: `https://www.mytutorai.app/blog/${post.slug}`,
...
export default function BlogPostPage({ params }: { params: { slug: string } }) {
  ...
  // Schema.org JSON-LD per articolo individuale
  const articleSchema = {
    ...
    mainEntityOfPage: {
      '@type': 'WebPage',
      '@id': `https://www.mytutorai.app/blog/${post.slug}`,
    },
    keywords: post.tags?.join(', '),
    ...
    itemListElement: [
      {
        ...
        position: 1,
        name: 'Home',
        item: 'https://www.mytutorai.app',
      },
      {
        '@type': 'ListItem',
        position: 2,
        name: 'Blog',
        item: 'https://www.mytutorai.app/blog',
      },
      {
        '@type': 'ListItem',
        position: 3,
        name: post.title,
        item: `https://www.mytutorai.app/blog/${post.slug}`,
      },
    ],
  };

  // FAQ Schema - Solo se l'articolo ha FAQ
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white shadow-sm pt-20">
        <div className="container mx-auto px-6 py-8">
          <div className="max-w-4xl mx-auto">
                         {/* Breadcrumb migliorato per maggiore visibilità */}
            ...
            {post.author && (
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">Team TutorAI</p>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="container mx-auto px-6 py-12">
        <div className="max-w-4xl mx-auto">
          ...
          {/* Social Captions - Nascosto per uso futuro */}
          {/* {post.socialCaptions && (
            <div className="mt-8 p-4 bg-gray-50 rounded-lg">
              <h4 className="text-sm font-semibold text-gray-900 mb-2">Condividi questo articolo:</h4>
              <div className="space-y-2 text-xs text-gray-600">
                {post.socialCaptions.linkedin && (
                  <div>
                    <strong>LinkedIn:</strong> {post.socialCaptions.linkedin}
                  </div>
                )}
                {post.socialCaptions.twitter && (
                  <div>
                    <strong>Twitter:</strong> {post.socialCaptions.twitter}
                  </div>
                )}
                {post.socialCaptions.instagram && (
                  <div>
                    <strong>Instagram:</strong> {post.socialCaptions.instagram}
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="mt-16 bg-gradient-to-r from-primary to-purple-600 text-white rounded-2xl p-8 text-center">
            <h2 className="text-2xl font-bold mb-4">Pronto a Trasformare il Tuo Apprendimento?</h2>
            <p className="text-lg mb-6 opacity-90">
              Scopri come TutorAI può personalizzare l'educazione per il tuo stile di apprendimento.
            </p>
            <Link
              href="/beta-access"
